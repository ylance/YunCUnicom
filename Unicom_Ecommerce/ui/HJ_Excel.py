    ...
    def process_data(self, path_gener, key):
        ...
        for path in path_gener:
            ...
            with ExcelFile(path) as xls:
                data = read_excel(xls, sheet_name=0, index_col=None, na_values=['NaN'])
        ...
        result[key] =concat(df_list,ignore_index=True)
        return result

    def read_file(self):
        with ExcelFile(self.file_path) as xls:
            data =read_excel(xls, 0, index_col=None, na_values=['NaN'])
            result = concat([data['SERIAL_NUMBER'], data['FEE']], axis=1, ignore_index=True).rename(
                columns={0: '号码', 1: '出账收入'}).fillna(0)
        return result

    def profit(self, users, profit_data):
        merged_data = merge(users, profit_data, how='left', on='号码')
        merged_data['分成金额'] = where(merged_data['出账收入'] >= 16.66, 16.66, 0)
...
# dp = DivideProfit('D:/Desktop/test_04','D:/Desktop/test_05','D:/Desktop/出账明细201805账期宽带.xls','D:/Desktop')
# dp.process()